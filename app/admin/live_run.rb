...
=end
    ...
    columns id: "liverun" do
      ...
      column do 
        @workstations = Workstation.all.order(:url)
        @workstations.each do |w|
          panel "Tivoli Job #{w.name}" do
            while File.exist?(File.join(Rails.public_path, ("live_tiv_data_#{w.url}.lock")))
              sleep 0.25
            end
            File.readlines(File.join(Rails.public_path, ("live_tiv_data_#{w.url}.txt"))).last(10).each do |tiv| #.collect{|c| c.gsub!("\t\t", "").gsub!("\n", "").gsub("\t", " - ")[0..100]}.each do |tiv|
